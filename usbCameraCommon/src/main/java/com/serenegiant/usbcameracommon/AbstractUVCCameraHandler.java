...
abstract class AbstractUVCCameraHandler extends Handler {
	...
	static final class CameraThread extends Thread {
		...
		public void handleStartPreview(final Object surface) {
			...
			try {
				Size bestPreviewSize = determineBestSize(mUVCCamera.getSupportedSizeList(),PREVIEW_SIZE_MAX_WIDTH );
				mUVCCamera.setPreviewSize(1280, 960, 1, 31, mPreviewMode, mBandwidthFactor);
...
//				boolean isDesireRatio = (size.width / 1) == (size.height / 1);
				boolean isBetterSize = (bestSize == null) || size.width > bestSize.width;
				if (isDesireRatio && isBetterSize) {
					bestSize = size;
					Log.d(TAG, " bestSize " + bestSize.height + " bestSize " + bestSize.width);
				}
			}

			if (bestSize == null) {
				Log.d(TAG, "cannot find the best camera size " + sizes.size());
